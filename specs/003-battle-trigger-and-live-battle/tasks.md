...
- [ ] T015 Write **failing** unit test: two same-owner companies at the same junction do NOT emit a battle trigger — extend `test/domain/use_cases/check_collisions_test.dart`
...
- [ ] T022 Wire `TickMatch` battle loop in `MatchNotifier.tick()` in `lib/state/match_notifier.dart`: pass `state.activeBattles` to `TickMatch.tick(activeBattles: ...)` and store `result.activeBattles` back into state; remove old `MatchPhase.inBattle` trigger-based transition
- [ ] T023 Add `MatchNotifier.advanceBattleRound(String battleId)` action in `lib/state/match_notifier.dart`: calls `BattleEngine.resolveRound` on the matching `ActiveBattle`, applies post-battle cleanup if resolved
- [ ] T024 Update `MatchDao.saveMatch` and `loadMatch` in `lib/data/drift/match_dao.dart` to persist and restore `activeBattles` using `BattlesTable`; map `battleId` column on companies (empty string → `null` on load)
...
- [ ] T025 Write **failing** unit test: `tick()` emits a `castleAssault` trigger when an attacking company arrives at an enemy castle with garrison companies — extend `test/domain/use_cases/check_collisions_test.dart`
- [ ] T026 Write **failing** unit test: `tick()` does NOT emit a battle trigger when attacking company arrives at an empty enemy castle, and castle ownership transfers immediately — extend `test/domain/use_cases/tick_match_test.dart`
- [ ] T027 Write **failing** unit test: `tick()` transfers castle ownership after `castleAssault` resolves with attacker win — extend `test/domain/use_cases/tick_match_test.dart`
- [ ] T028 Write **failing** unit test: a company arriving at a node where a `castleAssault` battle is already in progress joins as a reinforcement on the appropriate side — extend `test/domain/use_cases/tick_match_test.dart`

### Implementation for User Story 2

- [ ] T029 Add `castleAssault` variant to `CheckCollisions.check()` in `lib/domain/use_cases/check_collisions.dart`: detect arriving company at enemy castle, check for garrison companies; emit `BattleTrigger(kind: castleAssault)` with all defending garrison company IDs; skip battle if castle is empty
- [ ] T030 Handle `castleAssault` triggers in Phase A of `TickMatch.tick()` in `lib/domain/use_cases/tick_match.dart`: include all garrison companies as defenders when creating the `ActiveBattle`; set `attackerOwnership` correctly on `ActiveBattle`; confirm T025 turns green
- [ ] T031 Implement reinforcement joining in Phase A of `TickMatch.tick()` in `lib/domain/use_cases/tick_match.dart`: if an `ActiveBattle` already exists at the node, call `ResolveBattle.addReinforcement` and tag the new company with `battleId`; confirm T028 turns green
- [ ] T032 Implement castle ownership transfer in Phase C (post-battle cleanup) of `TickMatch.tick()` in `lib/domain/use_cases/tick_match.dart`: if `battle.kind == castleAssault` and outcome is attacker win, transfer `Castle.ownership` to `activeBattle.attackerOwnership`; confirm T027 turns green
- [ ] T033 Confirm empty-castle capture path is unaffected (no regression) — run `flutter test test/domain/` and verify T026 is green

**Checkpoint**: Castle-entry battle is fully working. Garrisoned castles trigger a battle; empty castles are captured immediately. Reinforcements join correctly. Castle ownership transfers on attacker win. All T025–T028 are green.
...
- [ ] T045 Wire "Next Round" button in `BattleScreen` in `lib/ui/screens/battle_screen.dart`: call `ref.read(matchNotifierProvider.notifier).advanceBattleRound(battleId)`; confirm T041 turns green; confirm T040 turns green
...
- [ ] T051 Profile game-loop tick with Flutter DevTools on an Android Snapdragon 665-class device: confirm single tick ≤ 16 ms with 2–5 active battles; capture DevTools screenshot for PR evidence
...
- [ ] T054 [P] Remove or document the unused global `battleNotifierProvider` path in `lib/state/battle_notifier.dart` (per R-005: retained but unused for new flow); add a `// TODO(cleanup): ...` comment or remove if safe
...
Phase 1 (Setup)
  └─► Phase 2 (Foundational) — BLOCKS everything
        ...
        │                             └─► Phase 8 (Polish)
        └─► (Phase 5 T038 BattleIndicator widget can be developed in parallel with Phase 3/4)
```
...
- **T034 + T035** (`BattleIndicator` widget tests) run in parallel with US1/US2 implementation
...
- **`[Story]`** labels (US1–US4) map directly to user stories in `spec.md` for full traceability.
